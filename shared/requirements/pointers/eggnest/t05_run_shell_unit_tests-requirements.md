# t05 Run Shell Unit Tests Wrapper Requirements

## Scope

Applies to `tests/t05_run_shell_unit_tests.sh`.
...
R015  Statement: Pointer delegates execution to the mapped runner shell-unit golden.
Design: Call `delegate_golden "tests/t05_run_shell_unit_tests.sh" "$@"` so the shim execs `${RUNNER_HOME}/tests/t05_run_shell_unit_tests.sh` with arguments passed through unchanged.
Tests:
