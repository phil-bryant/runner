...
src() {
  printf '%s' "${RUNBOOK_REPO_ROOT}/tests/t04_run_requirements_traceability_tests.sh"
}
...
@test "delegates to the mapped runner traceability golden" {
  #R015-T01: Verify the pointer calls delegate_golden "tests/t04_run_requirements_traceability_tests.sh" with "$@".
  run grep 'delegate_golden "tests/t04_run_requirements_traceability_tests.sh" "$@"' "$(src)"
  [ "$status" -eq 0 ]
}
